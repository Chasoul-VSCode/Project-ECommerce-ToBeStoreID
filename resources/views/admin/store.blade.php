...
<div class="container">
  <div class="row">
    @if(!isset($products) || empty($products))
      <div class="col-12">
        <p class="text-center">Belum ada barang yang tersedia</p>
      </div>
    @else
      @foreach($products as $index => $product)
        <div class="col-md-4 {{ $index >= 3 ? 'mt-4' : '' }}">
          <div class="card" style="width: 18rem;">
            @if (!empty($product['gambar_product']))
              <img class="card-img-top" src="{{ asset('images/' . $product['gambar_product']) }}" alt="Card image cap">
            @else
              <p class="text-center">No Image Available</p>
            @endif
            <div class="card-body">
              <h5 class="card-title">{{ $product['nama_b'] }}</h5>
              <p class="card-text">{{ $product['deskripsi'] }}</p>
              <p class="card-text"> Stock : {{ $product['stok_b'] }}</p>
              <a href="#" class="btn btn-primary">IDR {{ number_format($product['harga_b'], 0, ',', '.') }}</a>
            </div>
          </div>
        </div>
      @endforeach
...
<!-- Tambahkan di bawah kode HTML produk -->
<script>
